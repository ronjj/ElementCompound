...
struct MailView2: UIViewControllerRepresentable {
    ...
    class Coordinator: NSObject, MFMailComposeViewControllerDelegate {
                                   ...
                                   error: Error?)
        {
            ...
            if result == .sent {
            AudioServicesPlayAlertSound(SystemSoundID(1001))
            let alert = UIAlertController(title: "Thank You", message: "Your message has been sent.", preferredStyle: .alert)
                            let okAction = UIAlertAction(title: "OK", style: .default, handler: nil)
                            alert.addAction(okAction)
                            //self.present(alert, animated: true, completion: nil)
            }
    ...
    func updateUIViewController(_: MFMailComposeViewController,
                                context _: UIViewControllerRepresentableContext<MailView2>) {}
